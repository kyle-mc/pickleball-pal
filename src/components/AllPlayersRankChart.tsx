...
import { TIER_COLORS, TIER_BG_COLORS, TIER_ICONS, TIERS } from "@/lib/ranks";
...
export function AllPlayersRankChart({ players, highlightedPlayer }: AllPlayersRankChartProps) {
  ...
  const sortedPlayers = useMemo(() => [...players].sort((a, b) => b.mmr - a.mmr), [players]);
        ...
        <div className="space-y-4">
          <div className="relative">
            <div className="flex h-6 rounded-full overflow-hidden bg-muted/30 border border-border">
              {TIERS.map((tier, index) => {
                ...
                return (
                  <div key={tier.tier} className={cn("relative transition-all opacity-70", index > 0 && "border-l border-border/50")} style={{ width: `${width}%` }}>
            ...
            {sortedPlayers.map((player, index) => {
              ...
              return (
                <div key={player.name} className="absolute top-1/2 transform transition-all duration-500"
                  style={{ left: `${positionPercent}%`, transform: `translateX(-50%) translateY(-50%)`, zIndex: isHighlighted ? 20 : 10 - index }}>
                  <div className="relative flex flex-col items-center group cursor-pointer">
                    <div className={cn("w-6 h-6 rounded-full flex items-center justify-center text-xs shadow-lg border-2 transition-transform hover:scale-125",
                      isHighlighted && "scale-125 ring-2 ring-primary ring-offset-2 ring-offset-background")}
                      style={{ backgroundColor: color, borderColor: isHighlighted ? 'hsl(var(--primary))' : color }}>
          ...
          <div className="flex text-[9px] sm:text-[10px] text-muted-foreground mt-4">
            {TIERS.map((tier) => {
              const width = ((tier.maxMmr - tier.minMmr + 1) / MAX_MMR) * 100;
              return (
                <div key={tier.tier} className="text-center truncate" style={{ width: `${width}%` }}>
          ...
          <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-border">
            {sortedPlayers.map((player, index) => {
              ...
              const isHighlighted = highlightedPlayer === player.name;
              return (
                <div key={player.name} className={cn("flex items-center gap-1.5 px-2 py-1 rounded text-xs",
